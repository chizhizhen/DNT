...
class Layer {
 public:
  ...
  void ComputeContribution(vector<Blob<Dtype>*>* bottom);
  virtual void ComputeContribution_gpu(vector <Blob<Dtype>*>* bottom) {}
  virtual void ReduceTop(vector<Blob<Dtype>*>* bottom,  vector<Blob<Dtype>*>* top) {LOG(INFO) << "Layer::ReduceTop";}
  virtual void Reduce(vector<Blob<Dtype>*>* bottom,  const vector<Blob<Dtype>*>& top) {}
  ...
  inline void Backward2(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down,
      vector<Blob<Dtype>*>* bottom);
  virtual void Backward2_gpu(const vector<Blob<Dtype>*>& top,
...
// ComputeContribution warpers:
template <typename Dtype>
void Layer<Dtype>::ComputeContribution(vector <Blob<Dtype>*>* bottom) {
  switch (Caffe::mode()) {
    case Caffe::CPU:
    LOG(FATAL) << "Error: currently no cpu version of ComputeContribution is implemented!";
    break;
    case Caffe::GPU:
    ComputeContribution_gpu(bottom);
    break;
    default:
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
template <typename Dtype>
inline void Layer<Dtype>::Backward2(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down,
    vector<Blob<Dtype>*>* bottom) {
  switch (Caffe::mode()) {
  case Caffe::CPU:
    LOG(FATAL) << "Backward2_cpu is not implemented!";
    break;
  case Caffe::GPU:
    Backward2_gpu(top, propagate_down, bottom);
    break;
  default:
    LOG(FATAL) << "Unknown caffe mode.";
  }
