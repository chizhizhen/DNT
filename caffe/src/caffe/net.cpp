...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
template <typename Dtype>
void Net<Dtype>::ComputeContributions() {
  for (int i = 0; i < layers_.size(); i++) {
    //++++++++++++++
   // LOG(INFO) << "ComputeContributions";
    //++++++++++++++
    layers_[i]->ComputeContribution(&bottom_vecs_[i]);
  }
}
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
template <typename Dtype>
void Net<Dtype>::Reduce() {
  LOG(INFO) << "Begin Net::Reduce";
  int end = layers_.size()-1;
  layers_[end]->Reduce(&bottom_vecs_[end], top_vecs_[end]); // top relu layer
  layers_[end-1]->ReduceTop(&bottom_vecs_[end-1], &top_vecs_[end-1]); // the top conv layer
  for (int i = end-2; i >=0; i--) {
    layers_[i]->Reduce(&bottom_vecs_[i], top_vecs_[i]);
  }
}
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
template <typename Dtype>
void Net<Dtype>::Set_Select(const vector<shared_ptr<Blob<Dtype> > >& Select) {
  int num_conv_layers = 0;
  // bottom
  LOG(INFO) << "begin LOOP";
  int top_conv_ind = 0;
  for (int i = 0; i < layers_.size(); i++) {
    LayerParameter_LayerType layer_type = layers_[i]->type();
    if (layer_type != LayerParameter_LayerType_CONVOLUTION) {
      continue;
    }
    top_conv_ind = i;
    LOG(INFO) << "get channels";
    int channels = bottom_vecs_[i][0]->channels();
    LOG(INFO) << "get bottom";
    bottom_vecs_[i][0]->reset_select();
    Dtype* bottom_select = bottom_vecs_[i][0]->mutable_cpu_select();
    LOG(INFO) << "copy";
    caffe_copy(channels, Select[num_conv_layers]->cpu_data(), bottom_select);
    LOG(INFO) <<"bottom channels: " << Select[num_conv_layers]->asum_data() << " vs " << bottom_vecs_[i][0]->asum_select();
    num_conv_layers++;
  }
  // top of output layer
  LOG(INFO) << "get top channels"; 
  //int channels = top_vecs_[layers_.size()-1][0]->channels();
  int channels = top_vecs_[top_conv_ind][0]->channels();

  LOG(INFO) << "get top";
  top_vecs_[top_conv_ind][0]->reset_select();
  //Dtype* top_select = top_vecs_[layers_.size()-1][0]->mutable_cpu_select();
  Dtype* top_select = top_vecs_[top_conv_ind][0]->mutable_cpu_select();

  LOG(INFO) << "copy top";

  caffe_copy(channels, Select[num_conv_layers]->cpu_data(), top_select);
  LOG(INFO) << "done copy top";
